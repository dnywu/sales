...
{
    [Specification]
    public class When_correct_notes : OneEventTestFixture<CorrectNotes, NotesCorrected>
    {
        static Guid paymentId = Guid.NewGuid();
        static Guid paymentModeId = Guid.NewGuid();
        public When_correct_notes()
        {
        ...
        protected override CorrectNotes WhenExecuting()
        {
            return new CorrectNotes
            {
                InvoiceId = this.EventSourceId,
                PaymentId = paymentId,
                Notes = "correcting notes"
            };
        }
        ...
        {
            Assert.That(TheEvent.Notes, Is.EqualTo("correcting notes"));
        }
    }
}